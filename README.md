...
### Prijzen in de wedstrijdanalyse

De wedstrijdpagina vraagt bij Unibet alleen de gekoppelde wedstrijd op, via een competitiecatalogus en één eventaanvraag. Problemen bij andere wedstrijden kunnen daardoor niet meer de volledige Unibet-prijsaanvraag voor deze analyse laten mislukken. Koppeling blijft strikt op teams en aftrap; dezelfde teams op een andere datum worden uitgesloten. Dagoverzichten behouden de begrensde batchfeed.

De analyse kiest Unibet wanneer er prijzen zijn, anders een beschikbare vergelijkingsbookmaker met een expliciete melding. Odds zijn leesbare bedragen; handmatige inputs per markt zijn verwijderd. Alleen een optionele, uitklapbare invoer voor de door de bookmaker bevestigde gezamenlijke betbuilderprijs blijft bestaan. Andere bookmakers staan in een uitklapbare vergelijking. De tabel gebruikt uitgelijnde kolommen op desktop en kaarten op mobiel. `scripts/check-odds.ts` controleert echte prijzen én het gedrag bij ontbreken van Unibet-prijzen.

### Serie A en Ligue 1

Vier competities delen nu één configuratie: Premier League (`E0`), La Liga (`SP1`), Serie A (`I1`) en Ligue 1 (`F1`). Italië gebruikt `it.1.json`, `ita.1` en de gecontroleerde Unibet-groep `1000095001`; Frankrijk gebruikt `fr.1.json`, `fra.1` en groep `1000094991`. OpenFootball-tijden worden als lokale Rome-/Parijs-tijd geïnterpreteerd; CSV-tijden blijven Brits. Beide nieuwe competities zijn tegen de huidige CSV-aftrappen gecontroleerd, inclusief tests voor zomer- en wintertijd.

Het programma, competitie-filter, analyses, uitslagen/recaps, spelers, odds, spotlight en Combi x2–x3 ondersteunen beide toevoegingen. Clubnamen zijn expliciet gekoppeld voor vijf CSV-seizoenen, huidige programma's, ESPN en Unibet; alle 38 huidige clublogo's staan lokaal. De volledige schema's bevatten 380 Serie A- en 306 Ligue 1-wedstrijden. Beschikbaarheid van statistieken en prijzen blijft afhankelijk van de bron; de algemene oddsfeed houdt zijn bestaande maximum van 24 komende events, terwijl wedstrijdanalyses hun eigen Unibet-event opvragen.

De ochtend-/avondjob ververst automatisch alle vier competities. De gedeelde namespace is verhoogd naar `free-football:multi:v2` zodat oude overzichten met twee competities niet blijven hangen. `scripts/check-new-leagues.ts` controleert de echte bronnen; `scripts/seed-new-leagues.ts` vult beide competities en hun historiecache met servercredentials uitsluitend uit de procesomgeving. Geen nieuwe sleutels of migraties nodig.

### Compacte combihistorie

De meerdaagse combi bewaart in `markets-data:v3` elke wedstrijd één keer, met verwijzingen vanuit de teamreeksen. Alleen de maximaal twintig relevante voltooide duels per ploeg blijven nodig; volledige wedstrijdstatistieken, bronmetadata en H2H worden voor deze goalmarkten niet meegestuurd. De bestaande kwalificatie, scores en ruststanden blijven gelijk voor 5/10/20 en 80/90/100%. De normale wedstrijdanalyse behoudt haar volledige gegevens.

Cachelezingen filteren verlopen rijen al in Supabase, zodat grote verlopen documenten niet eerst worden gedownload. Met 53 komende wedstrijden daalde de gemeten historieopslag van 8.836 KB naar 818 KB. `scripts/check-market-speed.ts` meet echte gegevens met servercredentials uit de procesomgeving; tijden hangen van bron- en databasebeschikbaarheid af. De CSV-oddscache gebruikt v4 om ook eerder gecachete overzichten zonder Serie A/Ligue 1 te vervangen.

### Gedeelde korte geheugencache

Alle repository-cachelezingen voor wedstrijden, spotlightmodellen, analyses, spelers, odds en bronbestanden gebruiken een kleine cache per actieve serverinstantie. Deze bewaart maximaal 64 resultaten en 12 MB geserialiseerde data, maximaal 2 MB per resultaat. Hergebruik duurt hoogstens dertig seconden en nooit voorbij de oorspronkelijke databasevervaltijd. Minst recent gebruikte resultaten worden verwijderd; lege resultaten en fouten worden niet lokaal gecachet. Succesvolle writes vervangen de lokale waarde; de tabellen hebben afzonderlijke sleutels. Locks, rate limits en authenticatie blijven via de bestaande databasepaden lopen.

Dit vermindert herhaalde databaseaanvragen, maar een nieuwe serverinstantie of eerste bronberekening kan nog langer duren. Externe cache-invalidatie kan maximaal dertig seconden later zichtbaar worden. `scripts/check-general-speed.ts` meet twee echte serveraanvragen per onderdeel; de eerste aanvraag en de herhaalde aanvraag worden afzonderlijk gerapporteerd. Databasetijden en gemeten serverlokale tijden zijn geen gegarandeerde browserlaadtijden.

### Programma met wedstrijdodds

De oude sectie ‘Odds & combi’ met historische 100%-selecties is verwijderd. De afzonderlijke Combi x2–x3 blijft bestaan. In het programma staan bij komende wedstrijden de drie 1X2-prijzen (thuis/gelijk/uit) van één herkenbare bookmaker: Unibet indien beschikbaar, anders een beschikbare vergelijkingsbron. Er worden geen prijzen van verschillende bookmakers binnen één rij gemengd. Onbekende prijzen blijven een streepje; CSV-prijzen zijn als momentopname aangeduid. De minimumodd 1,10 geldt alleen voor combiselecties, niet voor de gewone wedstrijdprijzen.

`GET /api/odds?date=YYYY-MM-DD` haalt alleen de dagprijzen op, zonder historie of marktanalyse. Eén aanvraag bedient alle programmarijen; datumwissels annuleren de vorige aanvraag en odds blokkeren het laden van de wedstrijdlijst niet. Historische of gestarte wedstrijden tonen geen pre-matchodds. Doorklikken opent de bestaande uitgebreide wedstrijdanalyse. `scripts/check-program.ts` controleert echte odds, doorklikken, desktop/mobiel en het ontbreken van de oude historieaanvraag.
